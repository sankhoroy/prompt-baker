...
class PromptBakerOptimizer:
    def __init__(
        ...
        self._prompt_cache: dict[tuple[str, bool], list[str]] = {}
    ...
    def optimize(self, dataset_csv: str | Path, verbose: bool = False) -> CandidateGene:
        ...
        for generation in generation_iter:
            evaluated = self._evaluate_population(
                population=population, dataset=dataset, generation=generation, verbose=verbose
            )
            ...
            self.logger.log_event(
                ...
                {"generation": generation, "best_score": evaluated[0].score, "best_metrics": evaluated[0].metrics},
            )
    ...
    def _expand_prompts(self, prompts: list[str], prompt_kind: str) -> list[str]:
        expanded: list[str] = []
        target_meta = self._system_prompt_meta if prompt_kind == "system" else self._user_prompt_meta
        for prompt in prompts:
            ...
            expanded.extend(self._paraphrase(original, concise=False, prompt_kind=prompt_kind))
            if self.config.token_length_optimisation:
                expanded.extend(self._paraphrase(original, concise=True, prompt_kind=prompt_kind))
        deduped = list(dict.fromkeys(item.strip() for item in expanded if item.strip()))
        return deduped

    def _paraphrase(self, prompt: str, concise: bool, prompt_kind: str) -> list[str]:
            ...
            if text:
                ...
                target_meta.setdefault(
                    text,
                    {
                        ...
                        "prompt_kind": prompt_kind,
                        "variant_type": "paraphrased_concise" if concise else "paraphrased",
                    },
                )
    ...
    @staticmethod
    def simple_paraphrase(prompt: str, concise: bool) -> str:
        if concise:
            return f"Briefly: {prompt}"
        return f"Rephrase: {prompt}"
    ...
    def _evaluate_population(
        self, population: list[CandidateGene], dataset: pd.DataFrame, generation: int, verbose: bool = False
    ) -> list[CandidateGene]:
        ...
        for idx, candidate in candidate_iter:
            ...
            self.logger.log_candidate(generation=generation, candidate_idx=idx, candidate=candidate)
    ...
    def _predict_dataset(self, candidate: CandidateGene, dataset: pd.DataFrame) -> tuple[list[str], list[str]]:
        ...
        for _, row in dataset.iterrows():
            ...
            user_prompt = self._render_user_prompt(candidate.user_prompt, row_input)
            predicted = completion_fn(candidate.system_prompt, user_prompt).strip()
    ...
    def _score_candidate(
        self, candidate: CandidateGene, y_true: list[str], y_pred: list[str]
        ...
        if self.config.task_type == "classification":
            ...
            base_score = classification_metric(y_true_norm, y_pred_norm, metric_name)
            ...
            if not self.judge_score_fn:
                raise ValueError("judge_score_fn is required for llm_as_judge metric.")
            base_score = llm_judge_metric(
                y_true, y_pred, self.judge_score_fn, scale_max=self.config.judge_scale_max
            )
        else:
            raise ValueError(f"Unsupported metric {metric_name} for task type {self.config.task_type}")

        score = base_score
        if self.config.token_length_optimisation:
            prompt_len = len(candidate.system_prompt) + len(candidate.user_prompt)
            penalty = min(0.5, prompt_len / 4000.0) * self.config.token_penalty_weight
            score = max(0.0, base_score - penalty)
        metrics = {"base_score": float(base_score), "final_score": float(score)}
        return float(score), metrics

    def _next_generation(self, ranked_population: list[CandidateGene]) -> list[CandidateGene]:
        elite = ranked_population[: self.config.elite_size]
        next_population: list[CandidateGene] = [self._clone_gene(item) for item in elite]
        while len(next_population) < self.config.population_size:
            parent1, parent2 = self._select_parents(ranked_population)
            child = self._crossover(parent1, parent2) if self._rng.random() < self.config.crossover_rate else self._clone_gene(parent1)
            if self._rng.random() < self.config.mutation_rate:
                child = self._mutate(child)
            next_population.append(child)
        return next_population[: self.config.population_size]

    def _select_parents(self, population: list[CandidateGene]) -> tuple[CandidateGene, CandidateGene]:
        tournament_size = min(4, len(population))
        choices = self._rng.sample(population, tournament_size)
        choices.sort(key=lambda gene: gene.score or 0.0, reverse=True)
        return choices[0], choices[1 if len(choices) > 1 else 0]

    def _crossover(self, parent1: CandidateGene, parent2: CandidateGene) -> CandidateGene:
        system_prompt = parent1.system_prompt if self._rng.random() < 0.5 else parent2.system_prompt
        user_prompt = parent1.user_prompt if self._rng.random() < 0.5 else parent2.user_prompt
        model_name = parent1.model_name if self._rng.random() < 0.5 else parent2.model_name
    ...
    def _mutate(self, gene: CandidateGene) -> CandidateGene:
        system_pool = self._expand_prompts(self.system_prompts, prompt_kind="system")
        user_pool = self._expand_prompts(self.user_prompts, prompt_kind="user")
        ...
        mutated.metadata = self._build_candidate_metadata(mutated.system_prompt, mutated.user_prompt)
        return mutated
    ...
    def _build_candidate_metadata(self, system_prompt: str, user_prompt: str) -> dict[str, str]:
